...
int32_t Hi3516DaiStartup(const struct AudioCard *card, const struct DaiDevice *device)
{
    ...
    for (int i = 0; i < itemNum; i++) {
        int ret = AudioDaiRegUpdate(device, &regCfgItem[i]);
        if (ret != HDF_SUCCESS) {
...
static int32_t SetIISRate(const struct DaiDevice *device, struct AudioMixerControl *regCfgItem, uint32_t itemNum)
{
    ...
    regCfgItem[0 + shift].value = mclkSel;
    if (AudioDaiRegUpdate(device, &regCfgItem[0 + shift]) != HDF_SUCCESS) {
        AUDIO_DRIVER_LOG_ERR("set frequency fail.");
        return HDF_FAILURE;
    }
    ...
    regCfgItem[1 + shift].value = bclkRegVal;
    if (AudioDaiRegUpdate(device, &regCfgItem[1 + shift]) != HDF_SUCCESS) {
        AUDIO_DRIVER_LOG_ERR("set frequency fail.");
        return HDF_FAILURE;
    }
    return HDF_SUCCESS;
...
int32_t DaiParamsUpdate(const struct DaiDevice *device)
{
    ...
    itemNum = device->devData->regConfig->audioRegParams[AUDIO_DAI_PATAM_GROUP]->itemNum;

    if (device->devData->pcmInfo.streamType == AUDIO_CAPTURE_STREAM) {
        shift = shiftMax;
    }

    if (SetIISRate(device, regCfgItem, itemNum) != HDF_SUCCESS) {
    ...
    regCfgItem[index2 + shift].value = value;
    if (AudioDaiRegUpdate(device, &regCfgItem[index2 + shift]) != HDF_SUCCESS) {
        AUDIO_DRIVER_LOG_ERR("set bitWidth fail.");
        return HDF_FAILURE;
    }

    value = device->devData->pcmInfo.channels - 1;
    regCfgItem[index3 + shift].value = value;
    if (AudioDaiRegUpdate(device, &regCfgItem[index3 + shift]) != HDF_SUCCESS) {
