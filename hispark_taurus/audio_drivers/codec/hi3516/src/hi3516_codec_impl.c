...
int32_t Hi3516CodecRegBitsUpdate(struct AudioMixerControl regAttr)
{
    ...
    ret = Hi3516CodecRegBitsRead(&regAttr, &oldValue);
    if (ret != HDF_SUCCESS) {
        ADM_LOG_ERR("Hi3516CodecRegBitsRead failed, ret=%d.", ret);
        return HDF_FAILURE;
    }
    ...
    struct CodecDaiParamsVal codecDaiParamsVal)
{
    ...
    ret = Hi3516CodecRegBitsUpdate(regAttr[itemNum - 1]);
    if (ret != HDF_SUCCESS) {
...
int32_t Hi3516CodecSetAdcTuneEnable(struct AudioRegCfgGroupNode **regCfgGroup)
{
    ...
    ret = Hi3516CodecRegBitsUpdate(regAttr[0]);
    if (ret != HDF_SUCCESS) {
...
int32_t Hi3516CodecAiaoGetCtrlOps(const struct AudioKcontrol *kcontrol, struct AudioCtrlElemValue *elemValue)
{
    uint32_t curValue;
    uint32_t rcurValue;
    void *codecVir = NULL;
...
int32_t Hi3516CodecAiaoSetCtrlOps(const struct AudioKcontrol *kcontrol, const struct AudioCtrlElemValue *elemValue)
{
    uint32_t value;
    uint32_t rvalue;
